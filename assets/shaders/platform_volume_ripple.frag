...
{
    float minExtent = min(halfExtents.x, min(halfExtents.y, halfExtents.z));
    float amplitude = max(minExtent * 0.045, 0.035);
    vec3 warpedPoint = point;

    warpedPoint.x += sin(point.y * 3.8 + point.z * 1.7 + t * 2.8) * amplitude;
    warpedPoint.y += sin(point.z * 4.6 + point.x * 1.4 + t * 2.1) * amplitude * 0.75;
    warpedPoint.z += sin(point.x * 4.2 + point.y * 1.9 + t * 3.1) * amplitude;
...
{
    float wave = sin(point.x * 2.8 + point.y * 2.0 + point.z * 3.2 - t * 5.0);
    wave += sin(point.x * 5.6 - point.z * 4.8 + t * 3.4) * 0.5;
    return 0.75 + wave * 0.25;
}
...
void main()
{
    ...
    for (int i = 0; i < VOLUME_STEPS; ++i)
    {
        ...
        float edgeDistance = distanceToInnerPrismEdges(warpedPoint, halfExtents);
        float halo = exp(-2.2 * edgeDistance / max(tubeRadius, 0.0001)) * ripple;
        float bloom = exp(-0.85 * edgeDistance / max(tubeRadius, 0.0001)) * (0.85 + ripple * 0.35);
        float core = exp(-9.5 * edgeDistance / max(tubeRadius, 0.0001)) * (0.8 + ripple * 0.45);
