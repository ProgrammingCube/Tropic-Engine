...
void main()
{
    ...
    shellColor *= mix(1.0, 0.22, outerCornerSoften);

    vec3 neonColor = shellColor + volumeColor;
    neonColor += vec3(0.10, 0.30, 0.12) * clamp(glowAmount, 0.0, 1.8);
    neonColor += vec3(0.24, 0.26, 0.09) * clamp(coreAmount, 0.0, 1.35);
    neonColor += vec3(0.05, 0.14, 0.06) * outerCornerSoften;
    neonColor = min(neonColor, vec3(1.0));

    float shellAlpha = (0.08 + fresnel * 0.07) * mix(1.0, 0.16, outerCornerSoften);
    float glowAlpha = clamp(glowAmount * 0.58 + coreAmount * 0.32 + outerCornerSoften * 0.10, 0.0, 0.82);
    float platformAlpha = clamp(shellAlpha + glowAlpha, 0.10, 0.84);
